...
func (i OidcHandler) Init() {
	verifier, config := initVerifierAndConfig(i)

	i.Config = config
	store := cookie.NewStore([]byte("secret"))
	store.Options(sessions.Options{MaxAge: 60 * 60 * 24}) // expire in 30 days
	i.Router.Use(sessions.Sessions("aruna", store))

	i.Router.GET("/login", loginHandler(i))
...
func initVerifierAndConfig(i OidcHandler) (*oidc.IDTokenVerifier, *oauth2.Config) {
...
func logoutHandler(i OidcHandler) func(c *gin.Context) {
...
func callbackHandler(i OidcHandler, verifier *oidc.IDTokenVerifier, config *oauth2.Config) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		serverSession.Set("aruna", rawIDToken)
...
func loginHandler(i OidcHandler) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		c.Redirect(http.StatusFound, i.Config.AuthCodeURL(state)) //redirect to authorization server
	}

}

func handleError(c *gin.Context, i OidcHandler, err error, message string) bool {
	if err == nil {
		return false
	}
...
func handleOk(c *gin.Context, i OidcHandler, ok bool, message string) bool {
	if ok {
		return false
	}
